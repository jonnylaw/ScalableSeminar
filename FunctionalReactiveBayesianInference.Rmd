...
# lightbulb_data %>%
#   filter(t >= as.POSIXct("2016-09-01 02:40:00"))
...
<!--     None, -->
<!--     OrnsteinParameter(mu = 1.0, sigma = 1.0) -->
<!-- ) -->
<!-- val times: List[Time] = // some times -->
<!-- val mod: UnparamModel = PoissonModel(stepBrownian) -->

<!-- simData(times, mod(p)) take 100 -->
<!-- ``` -->
...
<!-- * Now combine the Poisson model and the seasonal model: -->
...
<!-- ```scala -->
<!-- val poissonParams: Parameters = // Some Parameters -->
<!-- val seasonalParams: Parameters = // Some parameters -->

<!-- val params = poissonParams |+| seasonalParams -->
<!-- val unparamModel = PoissonModel(stepBrownian) |+| SeasonalModel(24, 3, stepOrnstein) -->
<!-- val model = unparamModel(params) -->
<!-- ``` -->
...
![](animatedParticleFilter.gif)

## Bayesian Inference With Stream
...
* First define the observation datatype:
```scala
case class Data(time: Datetime, observation: Double)
```
* Define the state of the bootstrap particle filter
```scala
...
```scala
def filter(init: FilterState): Flow[Data, FilterState] = {
  Flow[Data].scan(init)(filterStep)
}
```
