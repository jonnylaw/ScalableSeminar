...
* Online analysis of this data is required to understand this data
...
* Total Readings from sensor 03/11/2015: 316,142
</div>

## Urban Observatory Readings

```{r, eval=FALSE, echo=FALSE}
totalReadingsByTheme = tbl(my_db, sql("SELECT theme, COUNT(*) AS total_readings, COUNT(DISTINCT sensor_name) AS total_sensors, date_trunc('week', timestamp) AS date FROM sensor_data_formatted GROUP BY theme, date_trunc('week', timestamp)"))

totalReadingsByTheme %>%
  write.csv(file = "readings_by_theme.csv", row.names = FALSE)
```

```{r, echo=FALSE, message=FALSE, fig.align='center'}
totalReadingsByTheme = read_csv("readings_by_theme.csv")

p1 = totalReadingsByTheme %>%
  ggplot(aes(x = date, y = total_readings)) + 
  geom_line() + facet_wrap(~theme)

p2 = totalReadingsByTheme %>%
  ggplot(aes(x = date, y = total_sensors)) +
  geom_line() + facet_wrap(~theme)
...
*Let's describe a bootstrap filter for this Gaussian model:

$$\begin{align*}
y(t_i) &\sim \mathcal{N}(y(t_i) | x(t_i), V) \\
...
* There is a Scala library which implements composable POMP models and inference methods
