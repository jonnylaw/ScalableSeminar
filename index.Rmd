...
* In general, POMP models require simulation based filtering (such as the bootstrap filter [@Gordon1993] below) to determine the behaviour of the latent state. Assume we have a simple Gaussian Model
...
```scala
val stepState(mu: Double, sigma: Double): (State, TimeIncrement) => State = (x, dt) {
...
```scala
def filter(v: Double, init: FilterState) = Flow[Data].scan(init)(stepFilter(v))
...
* Parameter estimates are required to run the particle filter

* In order to account for the uncertainty of the parameter estimates, we can sample from the parameter posterior $p(\theta | y(1:T))$ and run multiple particle filters

* Running multiple particle filters (in parallel) is trivial:

```scala
val variance: Rand[Double] = // Posterior distribution for the observation variance, v
